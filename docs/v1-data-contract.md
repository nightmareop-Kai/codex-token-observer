...
- 初次采样从官方当前使用百分比开始；本版启用前的重置不追溯。
- 观察到到期前的额度重置时，累计已观测到的重置前用量。例如 80% 后重置，再用 30%，显示约 110%，不截断至 100%。
- 通过窗口到期时间变化、使用比例下降和可用重置次数减少判断重置。重置前最后一次采样可能漏掉消耗，因此累计值标记为估算（`≈`），不能保证还原软件停止期间的多次重置。
- 额度窗口自然到期后开始新一轮累计；手动重置造成的 carry 随账号分开持久化到 SQLite metadata。
- 读取失败显示上次值及 `STALE`，从未取得有效数据时显示 `—`；未知不能当作 0%。
- 颜色按累计百分比连续从绿色过渡到黄色、红色；达到 100% 后保持红色，并将 Token 数字的有效位提示为红色。
